...
from data.csv_dataset import csv_dataset
...
def create_dataset(dataset, config, is_train=True, min_scale=0.5):
    ...
    if dataset=='pretrain':
        ...
        dataset = csv_dataset(config['csv_file'], config['image_root'], transform_test)              
...
def create_loader_1(dataset, sampler, bs, n_worker, is_train, collate_fn):
    if is_train:
        shuffle = (sampler is None)
        drop_last = True
    else:
